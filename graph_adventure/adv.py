...
# `player.currentRoom.getExits()` and
...
traversal_map = {}
stack = Stack()
# keep looping until greater than targets
while len(traversalPath) < 2000:
    current_room = player.currentRoom.id  # starts at Room 0
    # these are the exits ['n','s','w','e']
    possible_exits = player.currentRoom.getExits()
    # if not seen process it ...
    if current_room not in traversal_map:
        exits = {}
        # initialize directions with "?"
        for direction in possible_exits:
            exits[direction] = "?"
        # add current_room exits to the map
        traversal_map[current_room] = exits

    # grab the exits for that current room
    exits = traversal_map[current_room]
    # CHOICES
    # Check unexplored exits , add to traversalPath
    # start from 0
    # check all direction if has been visited in map1
    # if not add it
    # also store the previous position
    # repeat
    # ------------------------------- W ------------------------------------
    if "w" in exits and exits["w"] == "?":
        player.travel("w")   # move player
        traversalPath.append("w")  # add to path\

        new_room = player.currentRoom.id

        exits["w"] = new_room

        if new_room not in traversal_map:
            new_room_exits = {}  # explore new room
            for direction in player.currentRoom.getExits():
                      # replace "?" to new_room "s"
                new_room_exits[direction] = "?"
            # store old room were just in the next room map, curent_room still set to old room
            # 0: {'n': '?', 's': '?', 'w': 7, 'e': '?'}, 7: {'w': '?', 'e': 0}
            new_room_exits["e"] = current_room
            # assign the exits for the new_room
            traversal_map[new_room] = new_room_exits
        else:
            # the room already exits in the graph
            traversal_map[new_room]["e"] = current_room
        # add direction to stack for backtrack
        stack.push("e")
    # ------------------------------- E ------------------------------------
    elif "e" in exits and exits["e"] == "?":
        player.travel("e")   # move player to south
        traversalPath.append("e")  # add to path\
        ...
        if new_room not in traversal_map:
            ...
            for direction in player.currentRoom.getExits():
                      # replace "?" to new_room "s"
                new_room_exits[direction] = "?"
            # set old room were just in, curent_room still set to old room
            new_room_exits["w"] = current_room
            # assign the exits for the new_room
            traversal_map[new_room] = new_room_exits
        else:
            # the room already exits in the graph
            traversal_map[new_room]["w"] = current_room
        # add direction to stack for backtrack
        stack.push("w")
    # ------------------------------- S ------------------------------------
    elif "s" in exits and exits["s"] == "?":
        player.travel("s")   # move player to south
        traversalPath.append("s")  # add to path\
        ...
        if new_room not in traversal_map:
            ...
            for direction in player.currentRoom.getExits():
                ...
                new_room_exits[direction] = "?"
            # set old room were just in, curent_room still set to old room
            new_room_exits["n"] = current_room
            # assign the exits for the new_room
            traversal_map[new_room] = new_room_exits
        else:
            # the room already exits in the graph
            traversal_map[new_room]["n"] = current_room
        # add direction to stack for backtrack
        stack.push("n")
    # ------------------------------- N ------------------------------------
    elif "n" in exits and exits["n"] == "?":
        player.travel("n")   # move player to south
        traversalPath.append("n")  # add to path\
        ...
        if new_room not in traversal_map:
            ...
            for direction in player.currentRoom.getExits():
                ...
                new_room_exits[direction] = "?"
            # set old room were just in, curent_room still set to old room
            new_room_exits["s"] = current_room
            # assign the exits for the new_room
            traversal_map[new_room] = new_room_exits
        else:
            # the room already exits in the graph
            traversal_map[new_room]["s"] = current_room
        # add direction to stack for backtrack
        stack.push("s")
    # ------------------------------- Backtrack ------------------------------------
    else:
        backtrack = stack.pop()
        print(
            f'{backtrack}, -------------------- backtrack -------------------- {stack.stack}')
        if backtrack is None:  # if stack is empty
            break
    ...
    print(
        f'---------------------------------Room {current_room} ---------------------------------')
    print(f"exits: -->         {exits}")
    print(f"traversal_map: --> {traversal_map}")
